...
// cores do tema escuro, caso queira alterar, so alterar aqui
  ...
  const handleSubmit = (event) => {
    ...
    const usuarios = [
      ...
      {
        email: "joao@galileu.com",
        password: "123456"
      }
    ];

// nessa  linha vai procurar o email que foi digitado no inpute vai comparar com os emails que estão no array
    const encontrarUsuario = usuarios.find((item) => item.email === email);



    if(encontrarUsuario == null) {
      setErroUsuario('algo de errado!')
    }
    else {
